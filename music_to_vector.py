...
def musicTxt_to_vector(style):
    v_list = []
    # 为了后续排查数据问题，这里还是分成两份，新数据没有合并
    filename = f'music_data/{style}-music.txt' # 添加了风格参数
    with open(filename) as file:
        contents = file.readlines()
        # 随机选择一行
        random_line = random.choice(contents)
        
        melody = random_line.split(' ')  # 将随机行分割为音符
        
        v = music_to_vector(melody)  # 转换为向量
        v_list.append(v)  # 添加到列表中
