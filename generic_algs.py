...
def fitness_function(melody: stream.Stream) -> float:
    ...
    note_pairs = [(melody.flatten().notes[i].pitch.name, melody.flatten().notes[i + 1].pitch.name)
                  for i in range(len(melody.flatten().notes) - 1) if
                  isinstance(melody.flatten().notes[i], note.Note) and
    ...
    weight_contour_smoothness = 1  # 旋律轮廓平滑性权重
    print('############################')
    print(weight_note * note_fitness)
    print(weight_interval * interval_fitness)
    print(weight_rhythm * rhythm_fitness )
    print(weight_range * range_fitness)
    print(weight_repetition * repetition_fitness)
    print(weight_duration_distribution * duration_distribution_fitness)
    print(weight_contour_smoothness * contour_smoothness_fitness)
    print('############################')
...
def run_generic_algorithm(melodies: list[stream.Stream], iterations=15, criteria=0.95, total=50) -> stream:
    iter = 0
